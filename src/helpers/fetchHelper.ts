...
export const getData = (): ReservationType[] => {
  const data = fetchData().reservations;
  const formattedData: ReservationType[] = [];
  for (let reservation of data) {
    let formattedReservation = {
      ...reservation,
      customer: `${reservation.customer.firstName} ${reservation.customer.lastName}`,
      businessDate: businessDateToDate(reservation.businessDate),
    };
    formattedData.push(formattedReservation as ReservationType);
  }
